...
RSpec.describe HarvestService, type: :service do
  ...
  before do
    ...
    allow_any_instance_of(FakeHarvestService).to receive(:get_project_entries).and_return(entries)
    allow_any_instance_of(FakeHarvestService).to receive(:get_users).and_return(users)
    allow_any_instance_of(FakeHarvestService).to receive(:get_project).and_return(project)
    allow_any_instance_of(FakeHarvestService).to receive(:get_tasks).and_return(tasks)
  end
  ...
  context '#sync_entries' do
    let(:subject) { service.sync_entries }
  ...
  context '#sync_users' do
    let(:subject) { service.sync_users }
  ...
  context '#sync_projects' do
    let(:subject) { service.sync_projects }
  ...
  context '#sync_tasks' do
    let(:subject) { service.sync_tasks }
